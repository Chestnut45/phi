...
ParticleEffectEditor::ParticleEffectEditor() : App("Particle Effect Editor", 4, 6)
{
    ...
    // Load materials
    scene.LoadMaterials("data/materials.pbm");
    ...
    if (ImGui::BeginMenuBar())
    {
        if (ImGui::BeginMenu("File"))
        {
            ...
            if (ImGui::MenuItem("Close"))
            {
                ...
                {
                    node->RemoveComponent<Phi::CPUParticleEffect>();
            ...
            if (ImGui::CollapsingHeader((emitter.name + "###").c_str(), &keepEmitter, ImGuiTreeNodeFlags_None))
            {
                ...
                if (ImGui::BeginCombo("Blend Mode", blendSelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(blendOptions); n++)
                    {
                        ...
                        {
                            emitter.blendMode = (Phi::CPUParticleEmitter::BlendMode)n;
                        }
                ...
                if (ImGui::BeginCombo("Spawn Mode", spawnSelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(spawnOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.spawnMode = (Phi::CPUParticleEmitter::SpawnMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                ...
                if (emitter.particleProperties.spawnMode == Phi::CPUParticleEmitter::SpawnMode::Continuous ||
                    emitter.particleProperties.spawnMode == Phi::CPUParticleEmitter::SpawnMode::ContinuousBurst)
                {
                    ImGui::DragFloat("Spawn Rate", &emitter.particleProperties.spawnRate, 0.001f, 0.0f, 65'536.0f);
                }
                else if (emitter.particleProperties.spawnMode != Phi::CPUParticleEmitter::SpawnMode::SingleBurst)
                {
                ...
                if (emitter.particleProperties.spawnMode == Phi::CPUParticleEmitter::SpawnMode::ContinuousBurst ||
                    emitter.particleProperties.spawnMode == Phi::CPUParticleEmitter::SpawnMode::SingleBurst)
                {
                    ImGui::DragInt("Burst Count", &emitter.particleProperties.burstCount, 1, 1, Phi::CPUParticleEmitter::MAX_PARTICLES);
                }
                else if (emitter.particleProperties.spawnMode == Phi::CPUParticleEmitter::SpawnMode::RandomBurst)
                {
                    ImGui::DragInt("Burst Count Min", &emitter.particleProperties.burstCountMin, 1, 1, Phi::CPUParticleEmitter::MAX_PARTICLES);
                    ImGui::DragInt("Burst Count Max", &emitter.particleProperties.burstCountMax, 1, 1, Phi::CPUParticleEmitter::MAX_PARTICLES);
                }
                ...
                if (ImGui::BeginCombo("Position Mode", positionSelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(positionOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.positionMode = (Phi::CPUParticleEmitter::PositionMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                }
                if (emitter.particleProperties.positionMode == Phi::CPUParticleEmitter::PositionMode::Constant)
                {
                    ImGui::DragFloat3("Position", &emitter.particleProperties.position[0], 0.001f, 0.0f, 0.0f);
                }
                else if (emitter.particleProperties.positionMode == Phi::CPUParticleEmitter::PositionMode::RandomMinMax)
                {
                ...
                if (ImGui::BeginCombo("Velocity Mode", velocitySelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(velocityOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.velocityMode = (Phi::CPUParticleEmitter::VelocityMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                }
                if (emitter.particleProperties.velocityMode == Phi::CPUParticleEmitter::VelocityMode::Constant)
                {
                    ImGui::DragFloat3("Velocity", &emitter.particleProperties.velocity[0], 0.001f, 0.0f, 0.0f);
                }
                else if (emitter.particleProperties.velocityMode == Phi::CPUParticleEmitter::VelocityMode::RandomMinMax)
                {
                ...
                if (ImGui::BeginCombo("Color Mode", colorSelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(colorOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.colorMode = (Phi::CPUParticleEmitter::ColorMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                ...
                {
                    case Phi::CPUParticleEmitter::ColorMode::Constant:
                        ImGui::ColorEdit3("Color", &emitter.particleProperties.color[0]);
                        break;
                    
                    case Phi::CPUParticleEmitter::ColorMode::RandomMinMax:
                ...
                if (ImGui::BeginCombo("Size Mode", sizeSelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(sizeOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.sizeMode = (Phi::CPUParticleEmitter::SizeMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                ...
                {
                    case Phi::CPUParticleEmitter::SizeMode::Constant:
                        ImGui::DragFloat2("Size", &emitter.particleProperties.size[0], 0.001f, 0.0f, 1024.0f);
                        break;
                    
                    case Phi::CPUParticleEmitter::SizeMode::RandomMinMax:
                    ...
                    case Phi::CPUParticleEmitter::SizeMode::RandomLerp:
                        ImGui::DragFloat2("Size Min", &emitter.particleProperties.sizeMin[0], 0.001f, 0.0f, 1024.0f);
                        ImGui::DragFloat2("Size Max", &emitter.particleProperties.sizeMax[0], 0.001f, 0.0f, 1024.0f);
                        break;
                    
                    case Phi::CPUParticleEmitter::SizeMode::LerpOverLifetime:
                ...
                if (ImGui::BeginCombo("Opacity Mode", opacitySelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(opacityOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.opacityMode = (Phi::CPUParticleEmitter::OpacityMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                ...
                {
                    case Phi::CPUParticleEmitter::OpacityMode::Constant:
                        ImGui::DragFloat("Opacity", &emitter.particleProperties.opacity, 0.001f, 0.0f, 1.0f);
                        break;
                    
                    case Phi::CPUParticleEmitter::OpacityMode::RandomMinMax:
                    ...
                    case Phi::CPUParticleEmitter::OpacityMode::LerpOverLifetime:
                ...
                if (ImGui::BeginCombo("Lifespan Mode", lifespanSelection))
                {
                    for (int n = 0; n < IM_ARRAYSIZE(lifespanOptions); n++)
                    {
                        ...
                        {
                            emitter.particleProperties.lifespanMode = (Phi::CPUParticleEmitter::LifespanMode)n;
                        }
                        if (is_selected) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                }
                if (emitter.particleProperties.lifespanMode == Phi::CPUParticleEmitter::LifespanMode::Constant)
                {
                    ImGui::DragFloat("Lifespan", &emitter.particleProperties.lifespan, 0.001f, 0.0f, 65'536.0f);
                }
                else if (emitter.particleProperties.lifespanMode == Phi::CPUParticleEmitter::LifespanMode::RandomMinMax)
                {
