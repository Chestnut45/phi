#include "voxel_mesh_splat_method.hpp"
...
namespace Phi
{
    VoxelMeshSplatMethod::VoxelMeshSplatMethod(const std::vector<Vertex>& voxels)
    {
        if (refCount == 0)
        {
            ...
            // Debug Logging
            Phi::Log("VoxelMeshSplatMethod resources initialized");
        }
    ...
    VoxelMeshSplatMethod::~VoxelMeshSplatMethod()
    {
        refCount--;
    ...
    void VoxelMeshSplatMethod::Render(const glm::mat4& transform, const glm::mat3& rotation)
    {
    ...
    void VoxelMeshSplatMethod::FlushRenderQueue()
    {
