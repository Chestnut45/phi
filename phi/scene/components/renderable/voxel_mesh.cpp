...
namespace Phi
{
    VoxelMesh::VoxelMesh()
    {
        if (refCount == 0)
        {
            ...
            shader = new Shader();
            shader->LoadShaderSource(GL_VERTEX_SHADER, "phi/graphics/shaders/voxel_mesh.vs");
            shader->LoadShaderSource(GL_FRAGMENT_SHADER, "phi/graphics/shaders/voxel_mesh.fs");
            shader->Link();
